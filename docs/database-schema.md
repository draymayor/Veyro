...
**Second real bug found and fixed (2026-09-04, found via deployed webhook testing, not a synthetic check):** this table was created empty and never backfilled with the derivation indices of addresses that already existed before the service switched to reading from it. Every "what's the next free index" lookup since then computed against an effectively-empty table, silently defaulting every brand-new user back to index 0 in whichever address group they touched first, a real, live, platform-wide collision risk across every address group with any pre-existing user (confirmed directly: a freshly created test user requesting a first-ever TRC20 address received the exact same address as an existing real user's TRX/USDT address). No cross-user collision existed yet in production data at the time this was caught (independently verified via direct query before and after the fix), this was a forward-looking landmine, not an active incident, but it would have collided on the very next new user in any of 15+ address groups. Fixed via a one-time backfill migration inserting one reservation row per existing real `(user_id, address_group, derivation_index)` combination, using the same `address_group` mapping `chain-config.ts`'s `CHAIN_CONFIGS` actually uses (EVM-family chains collapse to one shared group, everything else keyed by its own network), verified live: 8 real rows inserted, zero remaining groups with more than one user sharing an index. Worth noting for any future chain addition: this mapping is a static, one-shot mirror of `chain-config.ts`, it will silently drift if that file's grouping changes without a matching reconciliation, same class of risk as every other migration-drift incident this project has hit.
...
**Sweep threshold real-numbers correction, and a real, separate gas-solvency bug found while fixing it (2026-09-05):** the original `sweep_min_threshold_*` values (see `20260826230218_sweeper_tables.sql`) had drifted to near-zero in production at some point, genuinely too low to filter anything. Restored to real, verified numbers for BTC/LTC/DOGE/EVM-native/TRX from the original migration, and TRC20 retuned using real current TRON network-fee data (a real TRC20 transfer costs roughly 6.5-13.4 TRX today, sourced live, not guessed). `sweep_min_threshold_erc20_token` (stablecoins on ERC20/BEP20/Arbitrum/Optimism/Base) was replaced entirely rather than just retuned, since a flat USD-shaped number can't stay correct as Ethereum-family gas prices swing by an order of magnitude with congestion. Replaced with `sweep_fee_multiple_erc20_token` (default `5`): sweep only if the token balance's USD value exceeds 5× the live-estimated fee's USD value, evaluated at sweep time using a real live gas-price lookup and a new sweeper-local CoinGecko price feed (`price-feed.ts`, 60s cache, falls back to a stale price on a transient outage rather than failing the whole run), not a static pre-fetched number. While investigating this, a real, separate, more serious bug was found: `EvmAdapter.sweepToken()` had NO gas-solvency check at all, unlike `sweepNative()` and the consolidator's own token-payout path, which both correctly refuse if native gas can't cover the fee. Fixed as part of the same change, and made fail-closed: the sweep now throws if fee context is missing, rather than silently sweeping without the check, so a future wiring mistake can never quietly revert to the unprotected behavior.
...
**Second webhook provider added (2026-09-05): Alchemy, for EVM chains, Tatum kept for TRON only.** After confirming Tatum's shared 5-slot cap was too small for real usage (verified: 1 slot used tonight covers exactly one real user's one address), Alchemy's free tier (verified directly, twice, after two earlier real pricing mistakes were caught and corrected: 5 webhooks, each supporting up to 100,000 tracked addresses, 30M compute units/month, genuinely $0) was added specifically for EVM coverage. `user_crypto_addresses.alchemy_webhook_id` (nullable text) mirrors `tatum_subscription_id`'s exact semantics for EVM addresses. Real constraint discovered during design: Alchemy's Address Activity webhook is one network per webhook, so 5 webhooks covers 5 of the 14 EVM chains, not all of them. Activated: Ethereum mainnet, Polygon, BSC, Arbitrum, Base (chosen as the most likely to see real early user activity, not from real volume data, since none exists yet). The remaining 9 EVM chains stay on manual confirmation, easily activatable later (config-driven design, not hardcoded to 5) once a slot frees up or the plan is upgraded. A real, subtle bug was caught before building: Alchemy signs the raw request body bytes directly, unlike Tatum's guard which hashes the re-stringified parsed body, reusing Tatum's guard pattern would have silently broken verification, so Alchemy's receiver uses its own raw-body-aware guard.
...
**Genuine, independently-verified end-to-end proof (2026-09-04), the first time this entire feature has ever worked for real, real money, real chain, not synthetic**: 0.1 TRX sent to a real, live-generated deposit address on TRON mainnet. Confirmed via direct database query: `crypto_deposit_events.status = 'credited'`, `crypto_wallets.balance` for TRX correctly increased to `0.1`, a real linked `crypto_wallet_transactions` row (`type: 'webhook_deposit'`) exists, notifications fired without logged failure. Four real, independent bugs were found and fixed along the way to reach this, each confirmed via live evidence before being fixed, none guessed:
1. Tatum's subscription-creation chain-name enum differs entirely from its address/transaction-lookup chain slugs (e.g. `AVAX` → `avalanche-mainnet`, not `avax-mainnet`), empirically verified per-chain via a real create→read→delete cycle, not assumed to generalize from one chain's example.
2. The real webhook payload shape is completely different from what the code originally assumed (`to`/`from`/`value`/`tokenMetadata.symbol`/`chain: "tron-mainnet"`, not the assumed flat `address`/`amount`/`asset`/`chain: 'tron'`), confirmed via 3 real captured payloads before any fix was written.
3. The account-wide Tatum HMAC secret had drifted out of sync with what was stored in Secret Manager (root cause found by elimination: both JSON-canonicalization theories were tested against already-captured real data and ruled out, isolating the secret itself as the only remaining explanation), resolved by regenerating and re-syncing both sides.
4. A TRON block-lookup endpoint bug in the confirmation-depth poller, fixed alongside the above.

**Still open, by design, not yet tested**: whether a USDT-TRC20 (token, not native-coin) deposit on the same shared address correctly matches on `tokenMetadata.symbol`, a real, credible concern (grounded in Tatum's own documented example payload shapes, not yet directly observed for TRON), left deliberately untested until a real USDT-TRC20 transaction can confirm it either way, per the same "real evidence before fixing" discipline used throughout. A testnet path was investigated and genuinely ruled out, not for the reason initially assumed: `tron-testnet` is a real, valid Tatum subscription chain, but the project's `TATUM_API_KEY` is scoped to mainnet only, confirmed via a real `403 unsupported.apiKey.network.combination` response, a separate testnet account/key would be required, not a free or cheap workaround. **Zero risk while this stays open**: the existing fallback for any unmatched asset is "recorded for manual review," never an auto-credit guess, so an unresolved token-symbol match degrades safely to the pre-existing manual path, not a silent miscredit.

### `network_availability` (provider capacity/health tracking, added 2026-09-07)
```
id                     uuid (PK)
network_code           text        -- matches crypto_assets.network_code
provider               text        -- free-text, not enum: 'tatum' | 'alchemy' | 'blockchair' | 'trongrid' etc, deliberately not constrained so new providers don't need a migration
status                 text        -- 'available' | 'degraded' | 'unavailable' ('degraded' reserved for future use, doesn't gate deposits today, only 'unavailable' does)
reason                 text (nullable)   -- machine string, e.g. 'rate_limited', 'consecutive_failures', 'manual_override'
disabled_at            timestamptz (nullable)
auto_recovery_at       timestamptz (nullable)   -- earliest time the recovery cron may re-probe
consecutive_failures   integer
last_success_at        timestamptz (nullable)
last_failure_at        timestamptz (nullable)
created_at / updated_at
```
**Purpose**: real-time deposit safety, if a provider Veyro depends on for detecting a specific network's deposits becomes rate-limited or unavailable, that network is automatically hidden from users as a deposit option, so no one deposits into something that can't currently be tracked. Deliberately a SEPARATE table from `crypto_assets.is_active`, not an overload of it: `is_active` is an admin's deliberate business decision with no time dimension, `network_availability` is an automated, self-healing operational state. The actual gate combines both: `depositable = crypto_assets.is_active AND network_availability.status != 'unavailable'`. An admin's deliberate off is never touched by automation; a provider outage never overwrites an admin's setting either way.

**Every dependency (Tatum, Alchemy, Blockchair, TronGrid) requires self-tracking**, confirmed directly against each provider's real docs: none expose a live "X of Y used, resets at T" endpoint. Two of the four (TronGrid, Tatum) explicitly document that limits are subject to change without notice and should not be hardcoded into business logic; Blockchair publishes no ceiling at all, only a punitive 1-hour IP ban (confirmed directly, hit once during testing) for guessing wrong. This is why the safety mechanism is reactive (trips on an actual observed failure/429), not just a static counter compared against a hardcoded ceiling, a hardcoded number cannot be trusted as the sole safety gate against these specific providers.

**Trip logic**: any rate-limit-shaped response (429, etc.) trips immediately, one occurrence is enough. Other failures (timeouts, 5xx) trip after 3 consecutive failures within a short window. **Recovery, dual path, neither requires admin action**: (A) a scheduled cron (`@Cron('*/1 * * * *')`) probes `unavailable` rows past their `auto_recovery_at` with one cheap, real, known-safe call per provider, only flips back to `available` on a genuine successful response, never assumed; (B) any ordinary successful call from normal app traffic to that provider immediately clears the `unavailable` status too, faster than waiting for the next cron tick. Fail-closed if `network_availability` itself becomes unreachable: deposits block rather than assume everything's fine, a deliberate tradeoff favoring safety over availability.

Alerting reuses the existing `NotificationsService`/Resend pattern, a new `sendProviderHealthAlertEmail` fires only on state transitions (available→unavailable, unavailable→available), never per-failed-request, to avoid an alert storm during a sustained outage. Visible on the admin dashboard via a `providerHealth` block, same pattern as the existing `webhookCoverage` block.
...
### Support multi-ticket correction (2026-09-08)
Real design correction, worth preserving the reasoning: a first migration draft assumed `support_threads.id` didn't exist and needed adding, this was factually wrong, verified directly, `id` was already the primary key. The REAL blocker to multiple tickets per user was a separate, genuine `UNIQUE (user_id)` constraint (`support_threads_user_id_key`), independent of the primary key. Fixed correctly: dropped that unique constraint, added `support_messages.thread_id` (previously messages were only linked via `user_id`, with no way to distinguish which ticket a message belonged to, since only one ticket per user ever existed), backfilled unambiguously (every existing message's `user_id` mapped to exactly one pre-existing thread), made `NOT NULL`, and the insert policy now verifies the target thread is actually owned by the inserting user, not just that `user_id` matches (closing a real gap where a client could otherwise point `thread_id` at someone else's ticket).

**Real, live security gap found and fixed (2026-09-08), worth preserving**: the initial multi-ticket migration's policy-drop targeted a policy name that never existed yet (a naming mismatch from the earlier wrong draft), silently no-op'd, leaving BOTH the old, weaker policy (no thread-ownership check) and the new, correct one active simultaneously. Since Postgres RLS OR's permissive policies for the same command, the old policy alone was sufficient to let an insert through, meaning a client could set `thread_id` to any ticket ID and post into another user's support ticket, as long as their own `user_id` matched. Additionally, the reopen-on-new-message trigger was never actually updated to key by `thread_id`, still keyed by `user_id`, meaning one new message would have incorrectly reopened every resolved ticket that user had, not just the one it belonged to. Both fixed and independently verified directly: exactly one insert policy remains (the correct one), and the trigger now correctly scopes to `where id = new.thread_id`.

**Follow-up drift found and fixed separately (2026-09-09, [PR #20](https://github.com/draymayor/Veyro/pull/20))**: the trigger half of the fix above was never actually captured in migration history - the function that reads `where id = new.thread_id` was created, but nothing ever rewired the trigger to call it. The live database only behaved correctly because the missing DDL was run directly against it, out of band. A fresh environment applying only the originally-merged migration files would still have had the old, `user_id`-scoped trigger active. Caught during a later merge reconciliation by comparing migration files against actual live `pg_trigger`/`pg_policies` state rather than assuming the files matched reality; fixed with a new, idempotent migration.
